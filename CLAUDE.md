...
**Free (no ads):** 4 techniques (Box, 4-7-8, Coherence, Triangle), unlimited sessions, full history + all charts (weekly activity, calendar, technique breakdown, personal bests, mood distribution), streaks.
...
Unlocks: all 10 techniques, custom technique builder, all color themes, all badges, data export, Apple Health sync. Prices are set in App Store Connect / RevenueCat; the values above are the in-code fallback strings in `app/paywall.tsx`. `logStartTrial` fires for weekly/annual; the auto-renewal disclaimer (Apple 3.1.2c) shows for those two plans.
